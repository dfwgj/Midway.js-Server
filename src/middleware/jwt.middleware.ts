  ...
  resolve() {
    return async (ctx: Context, next: NextFunction) => {
      ...
      if (/^Bearer$/i.test(scheme)) {
        try {
          // 验证 token 是否有效
          const decoded = await this.jwtService.verify(token, this.jwtConfig.secret, { complete: true });
          // 如果 token 有效，继续请求
          ...
          if (error.message === 'ForbiddenError') {
            throw new httpError.ForbiddenError('You do not have the necessary permissions');
          }
